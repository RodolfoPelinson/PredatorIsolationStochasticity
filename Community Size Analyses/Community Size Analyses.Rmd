...
Lets plot it:
```{r Plotting ab pr, dpi = 150, fig.height=5, fig.width=5}
...
First, lets load the necessary data:
```{r loading data non pr}
data(com_SS2_SS3_non_predators_abundance)
```

Analysing the data:
```{r running predators ab, cache=T}
...
Now pairwise differences:
```{r  pw_abundances non pr, cache = T}
...
Lets plot it:
```{r Plotting ab non pr, dpi = 150, fig.height=5, fig.width=5}
